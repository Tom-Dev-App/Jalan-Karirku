...
export default function CheckoutElearning ({params}) {
    ...
    <Navbar/>
            ...
            <div className='flex flex-col items-start gap-4  p-8 bg-primer4 rounded-lg'>
                <div className='flex flex-col gap-2 px-4 pb-4 border-b border-black0 w-full'>
                    ...
                    <p className='text-[18px]'>{academyClass.title}</p>
                    <div className='flex gap-2'>
                        <Icon icon="bx:purchase-tag" className='text-black0' style={{ fontSize: '24px' }}/>
                        <span className='text-[18px] font-bold'>{`Rp.${academyClass.price.toLocaleString('id-ID')}`}</span>
                    </div>
                </div>
                <div className='flex flex-col gap-2 w-full px-4 pb-4 border-b border-black0'>
                    <div className='flex justify-between'>
                        <span className='text-[18px]'>Sub Total</span>
                        <span className='text-[18px]'>{`Rp.${academyClass.price.toLocaleString('id-ID')}`}</span>
                    </div>
                    <div className='flex justify-between'>
                        <span className='text-[18px]'>PPN (10%)</span>
                        <span className='text-[18px]'>{calculatePlusTax(academyClass.price)}</span>
                    </div>
                    <div className='flex justify-between'>
                        <span className='text-[18px] font-bold'>Total</span>
                        <span className='text-[18px] font-bold'>{calculatePlusTax(academyClass.price)}</span>
                    </div>
                </div>
